##Edited by NB 8/21/23
## Used with new naming scheme to run deeprank based on one set of embeds
import os
import sys
import glob
import h5py
...
class GraphHDF5(object):
    def __init__(
        self,
        pdb_path,
        pdbid,  ## added for __add_embedding to match new naming scheme
        ref_path=None,
        graph_type="residue",
        pssm_path=None,
        embedding_path=None,
        ...
        # add embedding to the hdf5 file
        self._add_embedding(outfile=outfile, pdbs=pdbs, embedding_path=embedding_path,identifier=pdbid)
    ...
    @staticmethod
    def _add_embedding(outfile, pdbs, embedding_path, identifier):
        ...
        for mol in mols:
            ...
            for i in range(len(residues)):
                ...
                pt_name = identifier + '.' + chainID + '.pt' ##now uses name with pdbid, not filename
