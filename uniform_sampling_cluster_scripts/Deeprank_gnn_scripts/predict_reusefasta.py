##predict_reusefasta.py

## Edited by NB 9/6/23
## includes functionality of predicted_edited2.py
## Also should be able to skip over pdbs that have no contacts 
##(needed to also comment out line 215 and uncomment line 214 in Dataset.py, should make no difference but og version is backed up)
## Needed to add bio.sequtils in order to have proper sequence embedding
## USE THIS IF ALL DECOYS HAVE SAME NUMBER OF RESIDUES TO SAVE EMBEDDING TIME
## MAKE SURE GRAPHGENMP_REUSEFASTA.PY IS MAIN GRAPHGENMP.PY
...
## Only using the 650million trained set, not the bigger one?
#GNN_ESM_MODEL = (
#    Path(__file__).parent
#    / "paper_pretrained_models/scoring_of_docking_models/gnn_esm/treg_yfnat_b64_e20_lr0.001_foldall_esm.pth.tar"
#)
GNN_ESM_MODEL="/gpfs/gibbs/pi/ohern/nb685/DeepRank-GNN-esm/paper_pretrained_models/scoring_of_docking_models/gnn_esm/treg_yfnat_b64_e20_lr0.001_foldall_esm.pth.tar"
TOKS_PER_BATCH = 4096
REPR_LAYERS = [0, 32, 33]
...
def pdb_to_fasta(pdb_file_path: Path, main_fasta_fh: TextIOWrapper, identifier: str) -> None:
    ...
    for chain_id in ["A", "B"]:
        ...
        seq_1letter=seq1(sequence)

        # Write the sequence to a FASTA file
#        root = re.findall(r"(.*).pdb", pdb_file_path.name)[0]
        root=identifier  ## now saves based off identifier, rather than full pdbname
...
def get_embedding(fasta_file: Path, output_dir: Path) -> None:
    ...
    TOK= token
    "token batching, in which batches are constructed not so that they contain the same number of sequences, but rather so that 
    they contain approximately the same number of tokens (so a batch could consist either of a large number of short sequences
    or a small number of long sequences"
    """
...
def create_graph(pdb_path: Path, workspace_path: Path, identifier: str,pdbid: str) -> str:
    """Generate a graph ...?"""
    log.info(f"Generating graph, using {NPROC} processors")

    outfile = str(workspace_path / Path(identifier+ "_graph.hdf5"))
...
def predict(input: str, workspace_path: Path, identifier: str) -> str:
    """Predict the fnat of a protein complex."""
    log.info("Predicting fnat of protein complex.")
    gnn = GINet
    target = "fnat"
    print("predict.py target is "+target)
    edge_attr = ["dist"]
    #
    threshold = 0.3
    ...
    output = str(workspace_path / Path(identifier+"_GNN_esm_prediction.hdf5"))
    # with nostdout():
    model = NeuralNet(
        input,
        gnn,
...
def parse_output(csv_output: str, workspace_path: Path, identifier: str) -> None:
    ...
    output_fname = Path(workspace_path, identifier+"_output.csv")
...
def main():
    ...
    identificator = args.pdbid + "-gnn_esm_pred"
    workspace_path = setup_workspace(identificator)
    new_chain_ids = ["A", "B"]

    ## Setup output file
    output_fname = Path(workspace_path, args.pdbid+"_output.csv")
    with open(output_fname, "a") as f:
        f.write("pdb_id,predicted_fnat\n")

    all_decoys = sorted([f for f in os.listdir(args.pdb_file) if os.path.isfile(os.path.join(args.pdb_file, f))])



    for decoy in all_decoys:
        pdb_file = args.pdb_file+decoy
        ...
        ## PDB to FASTA
        fasta_f = Path(workspace_path, args.pdbid+"_all.fasta")
        if Path.is_file(fasta_f)==False: ## now only makes new fasta if there isn't one
            with open(fasta_f, "w") as f:
                pdb_to_fasta(pdb_file_path=Path(pdb_file), main_fasta_fh=f,identifier=args.pdbid)

        ## Generate embeddings
        embedding_f1=Path(workspace_path,args.pdbid + '.' + new_chain_ids[0] + '.pt')
        embedding_f2=Path(workspace_path,args.pdbid + '.' + new_chain_ids[1] + '.pt')

        if Path.is_file(embedding_f1)==False and Path.is_file(embedding_f2)==False: ##now only embeds if there aren't .pt files matching pdbid
            get_embedding(fasta_file=fasta_f, output_dir=workspace_path)

        ## Generate graphs
        graph = create_graph(pdb_path=pdb_file.parent, workspace_path=workspace_path, identifier=decoy_id,pdbid=args.pdbid)

        ## Predict fnat, if there is missing data (such as no contacts), outputs error and goes to next decoy
        try:
            csv_output = predict(input=graph, workspace_path=workspace_path, identifier=decoy_id)
            parse_output(csv_output=csv_output, workspace_path=workspace_path, identifier=args.pdbid)
        except: 
            print("Error in prediction")
            with open(output_fname, "a") as f:
                f.write(decoy_id+",NaN\n")
            os.system(f"rm "+str(Path(workspace_path,decoy_id+'*')))
