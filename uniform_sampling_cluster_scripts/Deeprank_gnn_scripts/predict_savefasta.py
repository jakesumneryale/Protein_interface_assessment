## predict_keepfasta.py
## Edited by NB 10/2/23
## includes functionality of predicted_edited2.py except it doesn't use the same fasta/embed
## USE THIS WHEN DECOYS DON'T ALL HAVE THE SAME NUMBER OF RESIDUES
...
### USE WITH ORIGINAL GRAPHGENMP BACKUP
...
#GNN_ESM_MODEL="/gpfs/gibbs/pi/ohern/nb685/Retrained_DeepRank/hp_tuning/e30_c10_lr5e-6/treg_yFnat_b64_e30_lr5e-06.pth.tar"
TOKS_PER_BATCH = 4096
REPR_LAYERS = [0, 32, 33]
...
def create_graph(pdb_path: Path, workspace_path: Path, identifier: str) -> str:
    """Generate a graph ...?"""
...
def main():
    ...
    for decoy in all_decoys:
        ...
        ## PDB to FASTA
        fasta_f = Path(workspace_path, decoy+"_all.fasta")
        ...
        ## Generate graphs
        try:
            graph = create_graph(pdb_path=pdb_file.parent, workspace_path=workspace_path, identifier=decoy_id)
